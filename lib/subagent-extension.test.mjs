...
async function loadTools(runtime, getProfiles = () => []) {
  const tools = new Map();
  const sent = [];
  const extension = createSubagentExtension(runtime, getProfiles);
...
test("integrated extension removes a legacy extension that owns the same tools", () => {
  ...
  const result = preferPiWebSubagentExtension({
    extensions: [legacy, host, other],
    errors: [{ path: legacy.path, error: "duplicate" }, { path: other.path, error: "other" }],
  });

  assert.deepEqual(result.extensions.map((extension) => extension.path), [host.path, other.path]);
...
test("foreground Agent streams updates and returns the completed result", async () => {
  ...
  const result = await tools.get("Agent").execute(
    "tool-call",
    { subagent_type: "Explore", prompt: "Find the parser", description: "Find parser" },
    undefined,
...
test("background Agent returns immediately and sends one completion follow-up", async () => {
  let finish;
