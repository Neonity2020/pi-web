...
test("persisted subagent resources restore the exact isolated prompt and tools", () => {
  ...
  assert.deepEqual(readSubagentSessionResources(entries, "/tmp/project"), {
    profile: "reviewer",
    appendSystemPrompt: ["Review carefully.", "Inherited parent context."],
    tools: ["read", "grep"],
  });
...
test("legacy subagent resources use the current profile without enabling a missing profile's tools", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-subagents-"));
  const metadata = (profileName) => [{
    type: "custom",
    customType: SUBAGENT_META_TYPE,
    id: "meta",
    parentId: null,
  ...
  try {
    saveProjectSubagentProfile(cwd, profile({
      ...
      systemPrompt: "Use the legacy reviewer prompt.",
      tools: ["read"],
      enabled: false,
    }));
    assert.deepEqual(readSubagentSessionResources(metadata("legacy-reviewer"), cwd), {
      profile: "legacy-reviewer",
      appendSystemPrompt: ["Use the legacy reviewer prompt."],
      tools: ["read"],
    });

    const missing = readSubagentSessionResources(metadata("deleted-profile"), cwd);
    assert.equal(missing.profile, "deleted-profile");
    assert.deepEqual(missing.tools, []);
    assert.doesNotMatch(missing.appendSystemPrompt.join("\n"), /parent agent/i);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});
